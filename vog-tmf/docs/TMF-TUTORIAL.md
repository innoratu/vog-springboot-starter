...
getters and setters are generated by **Lombok**'s `@Getter`/`@Setter`
annotations, exactly as `vog-demo`'s entities already do. That entity is
stored in **H2**, the same in-memory database from the previous tutorial's
...
`category` can point to a parent `category` to form a hierarchy (e.g.
`vog-tmf`'s seeded "Business" category alongside top-level "Mobile" and
"Internet" categories). A `catalog` is the umbrella that organizes categories
